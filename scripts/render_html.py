...
def load_fund_names():
    ...
    """从 config/funds.json 读取基金名称映射；文件缺失/格式错误时回退内置默认（与 run_daily 一致）。"""
...
def render(report, history, out_path):
    ...
    purchase = report.get("purchase") or {}  # {code: {"status","limit"}} 申购限额
    ...
    for c in pred_cards:
        ...
        # 申购限额（东财 fund_purchase_em）：仅金额，暂停申购显示「暂停」
        ...
        if _pur.get("status") == "暂停申购":
            ...
            _purchase_html = '<div class="purchase"><span>申购限额</span><span class="amt paused">暂停</span></div>'
            ...
            _purchase_html = f'<div class="purchase"><span>申购限额</span><span class="amt">{_amt}</span></div>'

        else:

            _purchase_html = '<div class="purchase"><span>申购限额</span><span class="amt">不限购</span></div>'

        purchase_html = _purchase_html
    ...
    verified_recent = verified_all[-5:][::-1]  # 默认显示最近 5 条，最新在前
