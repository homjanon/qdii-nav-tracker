...
| 滚动 NNLS 动态权重 | walk-forward 60日非负最小二乘（**仅持仓成分**；汇率已实测无增益，见踩坑 11） | 实际持仓与披露差多少？（疑似调仓） |
...
>
> **汇率只进静态篮子（2026-09-25）**：静态项按 `wsum × 当日汇率变动` 折算（wsum = 持仓权重和 → 汇率敞口 = 持仓比例，经济含义明确）；**滚动 NNLS 不再把汇率作为独立特征** —— 实测去掉后 3 只基金 MAE 不变或微降，而拟合出的汇率权重极不稳定（中位 0~6.9、最高 8.6；敞口 >100% 属经济不可能值），详见踩坑 11。
...
| 数据 | 主源 | 备源/兜底 |
|---|---|---|
| 二十大持仓 / 全持仓档案 | 天天基金 F10（HTTP/1.1 直连；报告期与持仓**同一份 HTML** 解析） | **二十大缓存（缓存优先**：距上次实时校验 <7 天直接复用 → 0 请求；到期/旧缓存无期次才联网核对**）** + 全持仓档案 holdings_full.json（披露期 120 天） |
...
| 中国基金净值日历 | pandas-market-calendars（XSHG） | QDII 净值只在 A股交易日公布 → 门控判的是**「预测目标净值日」（= 美股最近收盘日 `us_last`）是否 A股交易日**：非 A股日 → 该日无净值 → 预测永远等不到验证 → 跳过。另有**长假回归首日保护**（基金最新净值 ↔ 美股最近收盘间隔 ≥2 个美股交易日 → 跳过当日预测：净值将一次反映多日美股累计，单日模型必大偏差），次日自动恢复。预测落库处另有**防御断言**：`pred_date` 非 A股交易日一律不落记录 |
...
> **汇率（USD/CNH）用途（2026-09-25 收敛）**：**只用于静态篮子的币种折算**（`wsum × 当日汇率变动`，敞口 = 持仓比例 wsum）；**不再作为滚动 NNLS 的特征**（实测去掉后精度不降，见踩坑 11）。
> 口径注意：**中行牌价是在岸官方价**（平滑，约一半交易日变动为 0），**东财 USDCNH 是离岸市场价** —— 两者**日收益相关仅 0.13**，不可互换使用（水平价差仅 ~0.04，差异在日变动而非水平）。
...
> 判断依据：`classify_market()` —— 纯字母=US；`285A`/`6857`=JP、`005930`/`000660`=KR；
> 5 位数字（0/1/2 开头）=HK；6 位数字（含主板/创业板/科创板）=CN。

> **为什么只有 US/JP/KR 做缺口补齐**：这三个市场主源是 yfinance（会有整行空值的 K 线）；HK/CN 主源是新浪，历史完整，不引入改动风险。
...
2. **先查持久化缓存 `output/.gap_repair_cache.json`（`symbol|date → close`）** —— 历史交易日的收盘价**不可变**，
   命中即 0 请求。这是关键：Yahoo 的空行是它侧的故障（如 2026-09-22 共 22 个标的同时中招），
   不落盘的话每天都会重新拉同一批历史值（实测 `found=26/repaired=22` 逐次完全相同，35 天窗口内天天重补）；
3. 未命中的才按各市场的补齐链联网补齐（同一源只请求一次，批量补多天）：
   - 美股：**新浪 → 腾讯日线 → 东财**（腾讯日线可补任意历史缺口，需带交易所后缀，见踩坑记录 3）
   - 日韩：**东财 → 腾讯快照**（新浪不覆盖日韩，放进链里是空操作）
4. **源级熔断**：同一源连续 3 次补不到 → 本次运行不再尝试该源（写入报告 `data_quality.sources_disabled`）；
5. 补不到 → 以 **NaN 占位**（保留日期），并登记 `unresolved`；
6. `analysis.predict_next()` 在预测前检查依赖标的近 4 天是否有未修复缺口 → 有则**跳过该基金当日预测并告警**（宁可不出预测，也不出错预测）。

报告新增 `data_quality` 字段（`counts`{`found`/`repaired`/`from_cache`/`unresolved`} / `sources_disabled` / `gap_cache_entries`），
运行日志与页面徽章（⚠️ 数据缺口 N）同步暴露。`repaired` 只统计**本次由网络新补**的，`from_cache` 为缓存复用数。

> **源顺序为何"新浪优先"**：云端实测 新浪补缺口 22/22 成功（1 次请求/标的），东财 push2his 仅 11/46（GitHub runner 出口 IP 被限流）。
> 把东财放首位会让每次失败都带重试+休眠，实测把分析步骤从 ~153s 拖到 ~479s。各源补的都是同一交易日的真实收盘，**顺序不改变结果**。
...
腾讯日线接口 `web.ifzq.gtimg.cn/appstock/app/usfqkline/get` 对美股**个股**必须带交易所后缀，
否则只返回 1~2 条废数据（`usASML` → 2 条；`usASML.OQ` → 320 条完整日线）。
后缀**不能靠猜**：实测 `usCIEN` 的正确码是 `CIEN.N`（NYSE），若按 `.OQ` 先试，会命中一个
**混入 2014 年数据、且恰好缺目标交易日**的错误序列 → 静默写入错误收盘。

**正确做法**：先请求快照 `qt.gtimg.cn/q=us<CODE>`，用其 **`[2]` 字段**拿权威代码
（如 `ASML.OQ` / `CIEN.N` / `.NDX`），再用它请求日线；指数（`usNDX`）无需后缀。
**"看似成功但内容是错的"比"失败"危险得多**——优先用接口返回的权威标识，别按命名规律猜。
...
### 8. run_daily 在报告写完之后抛异常 → workflow 的「Commit outputs」被跳过 → 结果全丢
...
**现象**：`data_quality` 连续多次完全相同（`found=26 / repaired=22`，补的标的清单逐字一致）
—— 同一批历史缺口在**每次运行都被重新联网补齐**。

**根因两层**：
1. Yahoo 的空行是**它侧的故障**（2026-09-22 共 22 个标的同时返回整行 None）；`keepna=True` 保留该行后，
   每次运行都判定"这里有缺口"，**判定必然为真，与运行频率无关**；
2. 补齐结果只写进**进程内存**（`_CACHE`），进程退出即丢 → 下一轮从头再来。
   `GAP_WINDOW_DAYS=35` 意味着该缺口会被重补约 35 天（≈21 请求/天）。

**修复**：补齐结果落盘到 `output/.gap_repair_cache.json`（键 `symbol|date`）。历史交易日收盘价**不可变**，
故缓存永久有效 —— 落盘后第二次起降为 **0 请求**。顺带解决另一个隐患：窗口期过后若不再补齐，
NaN 会永久留在序列里被 `rolling_nnls(window=60)` 的回归窗口吃掉（少 1–2 行样本）。

> **通用教训**：取"不会变的数据"（历史 K 线 / 已披露持仓 / 已结算净值）就必须落盘复用；
> 只放进程内存等于没缓存。**"每天重算同一批不变的历史"是最典型的浪费。**

### 11. 「有经济含义」≠「该进回归」：汇率在 NNLS 里的过拟合

**现象**：`daily_report.json` 里 **5/12 只基金的 NNLS 权重中汇率（FX）排第一**，取值
`2.70 / 2.57 / 2.19 / 1.89 / 0.86` —— 而汇率敞口的物理上限是 **100% 持仓（权重 ≤ 1）**，
`>1` 即经济不可能值。
...
| 证据 | 结果 |
|---|---|
| A/B 回测（3 只基金，rolling NNLS 同算法） | **去掉 FX 后 MAE 不变或微降**：0.816→0.816 / 0.991→0.930 / 0.651→0.648；方向命中不变 |
| 权重稳定性 | 018147 云端权重 **2.70** vs 本地同算法 **0.0**（权重不可复现）；024239 权重中位 **6.86**、**97.9% 期数 >1** |
| 因子解释力 | `corr(中行牌价日收益, 基金净值日收益)` = **0.001~0.084（≈0）**；两个权威汇率源日收益互相关仅 **0.13** |

**修复**：`rolling_nnls` **不再使用汇率特征**（权重表无 FX 键，维度 -1）；汇率仍在**静态篮子**中
（`wsum × fxr`，敞口 = 持仓比例，经济正确）—— 本次实测其对预测的影响仅 **−0.01~−0.02pp/日**
（静态项）、NNLS 项为零。

> **通用教训**：因子"有经济含义"不等于"该进回归"。判据应是 ① 去掉后回测是否变差
> ② 权重是否稳定且在合理范围 ③ 与目标的相关性是否显著。**三者都不满足时，它只是在拟合噪声**——
> 而 NNLS 无约束的非负最小二乘 + 近 60 日窗口尤其容易放大小样本噪声。
>
> **附带的安全设计**：本项目精度统计（方向命中 / MAE / 五维）**只看 `pred_static`**
> （`verify_history` 里 `hit`/`err` 均由 `pred_static` 计算），因此调整 NNLS 特征
> **不影响命中率统计与验证逻辑** —— 改动前先确认"哪个字段在承重"，能显著降低改动风险。

### 12. 「双重记账」：同一次请求被记两次，指标虚高一倍

**现象**：运行日志 `数据源汇总` 里 `中行牌价=✓2成功/19.4s`、`yf=113` —— 但实际只发了
**1 次 / 9.7s**（中行牌价）、**~57 次**（yf）。

**根因**：`_retry_call`（内层，用 `_src_key(label)` 归一为源名记账）与 `fallback_chain`
（外层，用 `name` 记账）对**同一次 fetch 各记一次**。证据：`✓ [中行牌价] 成功 (8056条)` 与
`✓ [汇率/中行牌价] 成功 (8056条)` **同一毫秒、同条数**。

**修复**：`fallback_chain` 记账前比较 `_src_total()` 基线，**仅在「内层未记账」时补记**
（`_src_total()` 统计 `SRC_STATS` 的 ok+fail 总量）。只改统计口径，**不影响取数与预测**。

> **通用教训**：包装器（retry / fallback）与被包装函数若**都做埋点**，统计必然翻倍。
> 做法：埋点只在**一层**做，或用"基线比对"判断内层是否已记。修好之前，耗时/计数只能看**相对值**。
