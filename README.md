...
| 指标 | 定义 | 本地实测（21条） |
|---|---|---|
| 方向命中率 | sign(预测) == sign(实际) | 85.7% |
| **幅度命中率 ±1pp** | \|误差\| ≤ 1.0pp | 81.0% |
| **综合精度命中** | 方向对 且 \|误差\|≤1.0pp | 71.4% |
| **区间覆盖率** | 实际 ∈ 预测 ± 1.28×MAE 误差带（80%置信） | 81% |
| MAE / RMSE / bias | 误差统计 | 0.70 / 0.99 / -0.48pp |

- **幅度命中率**比方向命中率更能反映预测精度（方向 91% 但幅度 63% 是常态，方向命中"不难"）
- **区间覆盖率**衡量预测可信度：实际值落在"预测±误差带"内的比例，目标 ≥80%（实测 81% ✓）
